...
url1 = "https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=%EC%98%A4%EB%8A%98+%EC%B2%9C%EC%95%88+%EB%82%A0%EC%94%A8"
...
#멜론 차트가져오기
def music():
    label1.config(text="지니 차트",font=15) #라벨 변경
    ...
    all_songs = soap3.find("tr",attrs={"songid":"92046524"})
    for num in range(1,11):
        
        song = all_songs.find("a",attrs={"class":"title ellipsis"}).get_text().strip()
        singer = all_songs.find("a",attrs={"class":"artist ellipsis"}).get_text().strip()
...
def game():
    ...
    #게임 무대 설정
    background = pygame.image.load(resource_path("Game/배경.png"))
    stage1 = pygame.image.load(resource_path("Game/내골대.png"))
    #상대 골대 다르게 설정
    stage2 = pygame.image.load(resource_path("Game/상대골대2.png"))
    ...
    #캐릭터 설정
    character = pygame.image.load(resource_path("Game/내캐릭터.png"))
    ...
    #공 설정
    ball = pygame.image.load(resource_path("Game/내공.png"))
...
def make():
    ...
    Button(frame1,text="지니 차트",width=15,height=3,command=music).pack(side="left",padx=10)
